    ...
    con.connect(function (err) {
      ...
      con.query(sql, function (err, result) {
        ...
        con.end(function (err) {
          ...
          res.writeHead(200, { 'Content-Type': 'text/html' });
          res.write('<head> <meta charset="utf-8" /> </head>')
          res.write("Data inserted into table");
          res.end();
        });
      });
    });
      ...
      con.query(sql, function (err, result, fields) {
        ...
        con.end(function (err) {
          ...
          result.forEach(line => {
            // let w = "";
            // fields.forEach(col => {
            //   w +=col.name+": "+ line[col.name]+", &nbsp"; 
            // });         
            // res.write(w+"<br>");
            res.write(JSON.stringify(line) + "<br>");
          });
          res.end();
        });
      });
      ...
      if (sql !== null) {
        con.query(sql, function (err, result) {
          ...
          con.end(function (err) {
            ...
            res.writeHead(200, { 'Content-Type': 'text/html' });
            res.write('<head> <meta charset="utf-8" /> </head>')
            res.write("Number of records deleted: " + result.affectedRows);
            res.end();
          });
        });
      }
